...
public class ShoppingOffers {
    ...
    public static void main(String[] args) {
        ShoppingOffers testObj = new ShoppingOffers();
        Integer[] priceArray = new Integer[]{2, 3, 4};
        Integer[][] specialArray = new Integer[][]{{1, 1, 0, 4}, {2, 2, 1, 9}};
        Integer[] needsArray = new Integer[]{1, 2, 1};

        List<Integer> price = Arrays.asList(priceArray);
        List<List<Integer>> special = new ArrayList<>();
        for (Integer[] a : specialArray) {
            special.add(Arrays.asList(a));
        }
        List<Integer> needs = Arrays.asList(needsArray);

        int cost = testObj.shoppingOffers(price, special, needs);
